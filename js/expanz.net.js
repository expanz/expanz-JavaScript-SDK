...
$(function() {
	...
	CreateSessionRequest : function(username, password, callbacks) {
		...
		SendRequest(RequestObject.CreateSession(username, password, appsite),
				parseCreateSessionResponse(callbacks));
	},

	GetSessionDataRequest : function(callbacks) {
		SendRequest(RequestObject.GetSessionData(expanz.Storage
				.getSessionHandle()), parseGetSessionDataResponse(callbacks));
	},
	...
	ReleaseSessionRequest : function(callbacks) {
		SendRequest(RequestObject.ReleaseSession(expanz.Storage
				.getSessionHandle()), parseReleaseSessionResponse(callbacks));
	}
	};
	...
	CreateSession : function(username, password, appsite) {
		return {
		data : buildRequest('CreateSessionX', XMLNamespace)(
				RequestBody.CreateSession(username, password, appsite)),
		url : 'CreateSessionX'
		};
	},
	...
	GetSessionData : function(sessionHandle) {
		return {
		data : buildRequest('ExecX', 'http://www.expanz.com/ESAService',
				sessionHandle)(RequestBody.GetSessionData()),
		url : 'ExecX'
		};
	},
	...
	CreateActivity : function(activity, style, sessionHandle) {
		return {
		data : buildRequest('ExecX', 'http://www.expanz.com/ESAService',
				sessionHandle)(RequestBody.CreateActivity(activity, style)),
		url : 'ExecX'
		};
	},
	...
	Delta : function(id, value, activity, sessionHandle) {
		return {
		data : buildRequest('ExecX', 'http://www.expanz.com/ESAService',
				sessionHandle)(RequestBody.Delta(id, value, activity)),
		url : 'ExecX'
		};
	},

	Method : function(name, contextObject, context, activity, sessionHandle) {
		return {
		data : buildRequest('ExecX', 'http://www.expanz.com/ESAService',
				sessionHandle)(
				RequestBody.CreateMethod(name, contextObject, context, activity)),
		url : 'ExecX'
		};
	},
	...
	DestroyActivity : function(activity, sessionHandle) {
		return {
		data : buildRequest('ExecX', 'http://www.expanz.com/ESAService',
				sessionHandle)(RequestBody.DestroyActivity(activity)),
		url : 'ExecX'
		};
	},
	...
	ReleaseSession : function(sessionHandle) {
		return {
		data : buildRequest('ReleaseSession', 'http://www.expanz.com/ESAService',
				sessionHandle)(RequestBody.CreateReleaseSession()),
		url : 'ExecX'
		};
	}
	};
	...
	CreateActivity : function(activity, style) {
		...
		center += activity.getAttr('key') ? ' initialKey="' + activity
				.getAttr('key') + '">' : '>';
			...
			_
					.each(
							...
							function(grid, gridId) {
								center += '<DataPublication id="' + gridId + '" populateMethod="' + grid
										.getAttr('populateMethod') + '"';
								grid.getAttr('contextObject') ? center += ' contextObject="' + grid
										.getAttr('contextObject') + '"' : '';
								center += '/>';
							});
		}

		if (activity.hasDataControl()) {
			_.each(activity.getDataControls(),
					function(DataControl, DataControlId) {
	...
	var parseCreateSessionResponse = function(callbacks) {
		return function apply(xml) {
			...
			if ($(xml).find('CreateSessionXResult').length > 0) {
				expanz.Storage.setSessionHandle($(xml).find('CreateSessionXResult')
						.text());
			} else {
				if (callbacks && callbacks.error) {
					callbacks
							.error("Error: Server did not provide a sessionhandle. We are unable to log you in at this time.");
				}
				return;
			}
	...
	function parseGetSessionDataResponse(callbacks) {
		return function apply(xml) {
			...
			var processAreas = [];
			$(xml).find('processarea').each(
					function() {
						var processArea = new ProcessArea($(this).attr('id'), $(this)
								.attr('title'));
						$(this).find('activity').each(
								function() {
									processArea.activities.push(new ActivityInfo($(this)
			...
			$.get('./formmapping.xml', function(data) {
				...
				$(data).find('activity').each(
						function() {
							...
							var gridviewList = [];
							$(this).find('gridview').each(
									function() {
										var gridview = new GridViewInfo($(this)
												.attr('id'));
										$(this).find('column').each(
												function() {
													gridview.addColumn(
															$(this).attr('field'), $(this)
																	.attr('width'));
												});
	...
	function parseCreateActivityResponse(activity, callbacks) {
		return function apply(xml) {
			...
			if (execResults) {
				...
				if (callbacks && callbacks.success) {
					callbacks
							.success('Activity (' + activity.name + ') has been loaded: ' + execResults);
				}

			} else {
				if (callbacks && callbacks.error) {
					callbacks
							.error('Server gave an empty response to a CreateActivity request: ' + xml);
				}
			}
			return;
		}
						...
						.each(
								function(data) {
									...
									if (id == 'picklist') {
										...
										expanz.Factory.bindGrids(activity, expanz.Views,
												expanz.Model, picklistWindow.windowEl
														.parent());
										...
										if (gridModel !== undefined) {
											...
											gridModel.updateRowSelected = function(selectedId, type) {
												console
														.log("From parseDeltaResponse:updateRowSelected id:" + selectedId + ' ,type:' + type);
												...
												expanz.Net.MethodRequest(
														'SetIdFromContext', contextObject,
														context, activity);
				...
				complete : function(HTTPrequest) {
					if (HTTPrequest.status != 200) {
						eval(responseHandler)(
								'There was a problem with the last request.');
					} else {
	...
	function fillGridModel(gridModel, data) {
		...
		_.each($(data).find('Column'), function(column) {
			gridModel.addColumn($(column).attr('id'), $(column).attr('field'), $(
					column).attr('label'), $(column).attr('datatype'), $(column)
					.attr('width'));
		});
