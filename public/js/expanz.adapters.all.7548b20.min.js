...
var columnCounter=0;_.each($(columnsXml).children(),function(columnXml){columnCounter++;if($(columnXml).attr("width")>0)gridDataTemplate+="<td><%=data["+columnCounter+"]%></td>"});gridDataTemplate+="</tr><\/script>";$("head").append(gridDataTemplate)}}var dataXml;var dataRows=new Array;if($(xml).find("Rows").length>0)dataXml=$(xml).find("Rows");_.each($(dataXml).children(),function(rowXml){var items=new Array;_.each($(rowXml).children(),function(cellXml){dataRows.push({name:$(cellXml).attr(labelAttribute),
category:$(rowXml).attr(labelAttribute),id:$(cellXml).attr(idAttribute)})})});listView.kendoMobileListView({dataSource:kendo.data.DataSource.create({data:dataRows,group:"category"}),template:"${name}",click:function(e){window.expanz.logToConsole(e.dataItem.id)}})};$(this).bind("publishData",publishData)};
