...
function renderKendoComponents(el){if(el===undefined)el=$("body");if(el.find("[renderingType=combobox]").length>0)el.find("[renderingType=combobox]").each(function(){$(this).KendoComboBoxAdapter()});if(el.find("[renderingType=dropdownlist]").length>0)el.find("[renderingType=dropdownlist]").each(function(){$(this).KendoDropDownListAdapter()});if(el.find("[bind=menu] > ul.menu").length>0)el.find("[bind=menu] > ul.menu").each(function(){$(this).kendoMenu()});if(el.find("[renderingType=numerictextbox]").length>
0)el.find("[renderingType=numerictextbox]").each(function(){$(this).kendoNumericTextBox({min:1,max:99,step:1,format:"n0"})})}function useKendoMobile(){expanz.UIplugin="kendoMobile"}
...
options["selectionCallback"];if(options["onSelection"]!==undefined)onSelection=options["onSelection"];if(options["parentSelectable"]!==undefined)parentSelectable=options["parentSelectable"]}var publishData=function(event,xml,view){window.expanz.logToConsole("KendoMobileListAdapter publishData");if(!$("head").find("#${name}")){var columnsXml;var gridDataTemplate;if($(xml).find("Columns").length>0)columnsXml=$(xml).find("Columns");if(columnsXml!==undefined){gridDataTemplate="<script type='text/template' id='${name}'><tr class='item listDisplay'";
var columnCounter=0;_.each($(columnsXml).children(),function(columnXml){columnCounter++;if($(columnXml).attr("width")>0)gridDataTemplate+="<td><%=data["+columnCounter+"]%></td>"});gridDataTemplate+="</tr><\/script>";$("head").append(gridDataTemplate)}}var dataXml;var dataRows=[];if($(xml).find("Rows").length>0)dataXml=$(xml).find("Rows");_.each($(dataXml).children(),function(rowXml){var items=[];_.each($(rowXml).children(),function(cellXml){dataRows.push({name:$(cellXml).attr(labelAttribute),category:$(rowXml).attr(labelAttribute),
id:$(cellXml).attr(idAttribute)})})});listView.kendoMobileListView({dataSource:kendo.data.DataSource.create({data:dataRows,group:"category"}),template:"${name}",click:function(e){window.expanz.logToConsole(e.dataItem.id)}})};$(this).bind("publishData",publishData)};
...
if(!isStatic){var elem;$(xmlData).find("["+labelAttribute+"]").each(function(){if(selectedText===$(this).attr(labelAttribute))elem=$(this)});if(onSelection)onSelection(view,elem.attr("id"),elem.is(childTag))}});if(runAfterPublish)runAfterPublish(xmlData);$(this).trigger("dataPublished")};$(this).bind("publishData",publishData)};
...
else{var regExp=new RegExp(filter,"i");newData=[];_.each(originalData,function(subData){var parentObj={};parentObj.text=subData.text;parentObj.expanded=subData.expanded;var items=[];_.each(subData.items,function(subItem){if(subItem.text.match(regExp))items.push(subItem)});if(items.length>0){parentObj.items=items;newData.push(parentObj)}})}treeView.html("");treeView.removeClass("k-widget k-treeview k-reset");treeView.kendoTreeView({dataSource:newData})};if(options){if(options["labelAttribute"]!==undefined)labelAttribute=
options["labelAttribute"];if(options["idAttribute"]!==undefined)idAttribute=options["idAttribute"];if(options["selectionCallback"]!==undefined)selectionCallback=options["selectionCallback"];if(options["onSelection"]!==undefined)onSelection=options["onSelection"];if(options["parentSelectable"]!==undefined)parentSelectable=options["parentSelectable"];if(options["expandedOnLoad"]!==undefined)expandedOnLoad=options["expandedOnLoad"];if(options["filteringInput"]!==undefined)filteringInput=options["filteringInput"];
...
else data.unshift({text:$(elem).attr("label")})});treeView.kendoTreeView({dataSource:data});treeView.data("kendoTreeView").bind("select",function(event){var selectedText=event.node.firstChild.textContent||event.node.firstChild.innerText;var isStatic=false;if(staticElements.length>0)_.each(staticElements,function(elem){if(selectedText===elem.label){isStatic=true;expanz.Net.MethodRequest(elem.method,[{name:"contextObject",value:elem.contextObject}],null,view.model.getAttr("parent"));$(treeView).trigger("TreeSelectionChanged",
{id:"static",text:selectedText})}});if(!isStatic){var elem;$(xmlData).find("["+labelAttribute+"]").each(function(){if(selectedText===$(this).attr(labelAttribute))elem=$(this)});if(onSelection)onSelection(view,elem.attr("id"),!elem.is(parentTag));$(treeView).trigger("TreeSelectionChanged",{id:elem.attr("id"),text:selectedText,parentId:elem.parent().attr("id"),parentText:elem.parent().attr(labelAttribute)})}});if(filteringInput&&filteringInput.length>0){var filterInputs=function(event){filterData(event,
$(this).val(),data)};var filterButtons=function(event){filterData(event,$(this).attr("filter"),data)};_.each(filteringInput,function(el){if($(el).is("input")){$(el).val("");$(el).bind("change",filterInputs)}else if($(el).is("button"))$(el).bind("click",filterButtons)})}if(runAfterPublish)runAfterPublish(xmlData)};$(this).bind("publishData",publishData);$(this).bind("filterData",filterData)};